-- Minimal DuckDB DDL for SQL-backed Delta metadata (PRD §7)
-- Notes: Use UUID, TIMESTAMP, JSON (DuckDB JSON extension) or VARCHAR fallback
...
CREATE TABLE IF NOT EXISTS dl_tables (
  ...
  properties JSON -- if JSON extension unavailable, change to VARCHAR
);
